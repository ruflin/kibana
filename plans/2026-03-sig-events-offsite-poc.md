...
#### 7a. Overview Page (Default Landing Tab)

Add an `overview` tab as the **first tab** and default landing page for SigDiscovery (`/_discovery/{tab}`):

Route: `/_discovery/overview` (default — invalid tabs redirect here)

Components:
- **Refresh button** — top-right, re-fetches all data (discoveries, suggestions, features, queries) in parallel.
- **Summary stats bar** — 5 `EuiStat` cards: total discoveries, suggestions / pending review (combined), features count, queries count, streams analyzed (unique streams across all discoveries/suggestions).
- **Top 3 Discoveries** — sorted by `relevance_score` descending (LLM-assigned 0–100 based on impact breadth 30%, evidence confidence 25%, novelty 25%, actionability 20%). Each shown as an `EuiCard` with severity badge, title, truncated description (150 chars), relevance score badge, and stream ref badges. "View all" link navigates to the Discoveries tab.
- **Top 3 Recommendations** — extracted from all discoveries' embedded `recommendations[]`, sorted by priority (critical > high > medium > low). Each shown as an `EuiCard` with priority badge, title, truncated description, step count, and source discovery title.
- **Top 3 Suggestions** — sorted by priority (derived from source discovery severity/relevance_score during Stage 3). Each shown as an `EuiCard` with type icon, priority badge, title, ES|QL code preview (60px), type label, and status badge. "View all" link navigates to the Suggestions tab.
- **Topology** — at the bottom, shows the persisted Mermaid topology diagram (loaded via `GET /internal/streams/_topology`). If no topology exists, shows a subdued panel directing the user to the Topology tab. "View full topology" link navigates to the Topology tab.
- **Empty state** — when no discoveries or suggestions exist, shows a welcome prompt with a link to the Discoveries tab.

Uses existing `GET /internal/streams/_discoveries`, `GET /internal/streams/_suggestions`, and `GET /internal/streams/_topology` endpoints. Features count via `useFetchFeatures` hook, queries count via `useFetchDiscoveryQueries` with `perPage: 1` (reads only `total`). Top 3 selection is client-side.

#### 7b. Discoveries Page

Add a `discoveries` tab to the SigDiscovery page (`/_discovery/{tab}`):
...
#### 7c. Suggestions Page (Inside SigDiscovery)
...
Components:
- **MermaidDiagram** — exported reusable component that renders Mermaid code via lazy-loaded `mermaid` npm package. Direct DOM rendering for `getBBox()` compatibility. Includes fullscreen modal (90vw × 85vh) with scaling SVG. Used by both the Topology tab and the Overview tab.
- **"Generate topology" / "Regenerate" button** — triggers LLM generation. Does NOT auto-generate on tab open — loads persisted topology first.
- **Loading state** — spinner while loading persisted topology from the server.
- **Empty state** — when no persisted topology exists, shows prompt with "Generate topology" button.
- **Error state** — falls back to raw Mermaid source display if rendering fails.

Server-side:
- `GET /internal/streams/_topology` — reads persisted Mermaid code from the discovery settings saved object (`topologyMermaid` field). Returns `{ mermaid: string | null }`.
- `POST /internal/streams/_topology` — generates a new topology diagram via LLM and **persists** the result to the discovery settings saved object. Fetches all features across all streams via `featureClient.getAllFeatures()`, summarizes them, sends to LLM via `inferenceClient.chatComplete()` with a system prompt instructing Mermaid `graph TD`/`graph LR` generation. Returns raw Mermaid code. The UI strips markdown fences and renders via lazy-loaded `mermaid` npm package.

Persistence: The `topologyMermaid` field is stored in the `stream-discovery-settings` saved object (same SO used by the Settings page for connector IDs). This means the topology survives page reloads and is shared across the Topology tab and Overview page.

#### 7e. Updated Route Config

```typescript
// In routes/config.tsx, update discovery tabs:
tabs: ['overview', 'streams', 'features', 'queries', 'discoveries', 'suggestions', 'topology', 'settings']
```

**Commit plan:**
- Commit 22: Route config + tab additions (overview, discoveries, suggestions, topology)
- Commit 23: OverviewTab (summary stats, top 3 discoveries/recommendations/suggestions) + DiscoveriesTable + DiscoveryDetailFlyout
...
| 2026-03-03 | WS7 | Overview tab added as default landing page | New `overview` tab is the first tab and default redirect target. Shows summary stats (4 EuiStat cards), top 3 discoveries (by relevance_score), top 3 recommendations (extracted from all discoveries' embedded recommendations, sorted by priority), and top 3 suggestions (by priority, with ES|QL preview). No new server API — reuses existing `GET _discoveries` and `GET _suggestions` endpoints, slicing top 3 on the client. |
| 2026-03-03 | WS7 | Topology persistence via saved object | Topology Mermaid code was regenerated on every tab open (expensive LLM call). Added `topologyMermaid` field to the `stream-discovery-settings` saved object. New `GET /internal/streams/_topology` reads persisted code; `POST` generates and persists. Topology tab now loads persisted diagram first and only regenerates on explicit button click. Overview page also reads the persisted topology for its bottom section. |
| 2026-03-03 | WS7 | Overview tab enhanced with Refresh, stats, and topology | Added Refresh button (re-fetches all data in parallel), expanded stats bar from 3 to 5 cards (added features count via `useFetchFeatures`, queries count via `useFetchDiscoveryQueries` with `perPage: 1`), added persisted topology diagram at the bottom, changed suggestions from vertical panels to horizontal `EuiCard` layout matching discoveries/recommendations. |
...
| 2026-03-03 | WS2 | Discovery pipeline converted from single-shot to reasoning agent with tools | The pipeline previously used `inferenceClient.prompt()` with forced `toolChoice: submit_discoveries` — a passive summarizer. Now uses `executeAsReasoningAgent` from `@kbn/inference-prompt-utils` with 8 internal pipeline tools: `get_sig_events_with_change_points` (change point analysis — primary tool), `get_log_patterns` (categorize\_text aggregation), `run_log_rate_analysis` (baseline vs deviation comparison), `search_events` (ES\|QL execution), `get_query_results` (query-specific results), `get_stream_features` (feature context), `search_discoveries` (semantic search for meta-discoveries), `get_query_definitions` (query definitions). Stage 1 uses max 8 steps, Stage 2 uses max 6 steps with a subset of tools. The LLM now actively investigates data before producing discoveries. |
| 2026-03-03 | WS2 | Cross-references now populated from evidence | `feature_refs` and `query_refs` were hardcoded to `[]`. Now extracted from discovery evidence: `feature_refs` from `evidence[].feature_name`, `query_refs` from `evidence[].query_title`. `stream_refs` are per-discovery (from `evidence[].stream_name`) instead of all-streams. |
| 2026-03-03 | WS3 | ES\|QL validation added to suggestion generation | Suggestions with invalid ES\|QL syntax are now rejected before persistence. Uses `parse()` from `@kbn/esql-language` to validate query syntax. `investigation` type suggestions (no ES\|QL) are exempt from validation. |
| 2026-03-03 | WS2 | Pipeline requires `scopedClusterClient` and `featureClient` | `generateDiscoveries()` signature expanded to accept `scopedClusterClient: IScopedClusterClient` and `featureClient?: FeatureClient`. These are needed by the internal pipeline tools (`get_sig_events_with_change_points` requires `scopedClusterClient` for the change point aggregation; `get_stream_features` requires `featureClient`). Task definition and Agent Builder tool updated to pass these dependencies. |
| 2026-03-03 | WS7 | `useQueriesApi` shared `AbortSignal` caused "signal is aborted without reason" on promote | `useAbortController()` creates a single `AbortSignal` tied to component lifecycle. All API methods (including mutations like `promote`) shared this signal. When React Query invalidation triggered a re-render after a successful promote, the signal was aborted, cancelling in-flight requests. Fix: removed `signal` from all mutation methods (`promote`, `promoteAll`, `upsertQuery`, `removeQuery`); retained only for the read operation (`getUnbackedQueriesCount`). **Lesson:** Mutations (user-initiated write operations) should never use lifecycle-bound abort signals — they must complete regardless of component unmount. |
| 2026-03-03 | WS7 | "Regenerate features" button added to Features tab | Features tab previously had no way to re-run feature identification. Added a "Regenerate features" button that extracts unique stream names from existing features, then triggers `POST /internal/streams/{streamName}/onboarding/_task` with `steps: [OnboardingStep.FeaturesIdentification]` for each stream via `pMap` (concurrency 5). Button is disabled when no AI connector is selected or no features exist. Auto-refetches features list after 10 seconds. |
| 2026-03-03 | WS7 | "Regenerate queries" button added to Queries tab | Same pattern as features regeneration. Extracts unique stream names from existing queries, triggers onboarding with `steps: [OnboardingStep.QueriesGeneration]` per stream. Uses `pMap` concurrency 5. Translations added to `translations.ts`. |
| 2026-03-03 | WS7 | Discovery detail flyout: markdown rendering for description | Description field was rendered as plain text. Changed from `<EuiText><p>...</p></EuiText>` to `<EuiMarkdownFormat>` so markdown (bold, lists, code blocks, headers) renders properly. |
| 2026-03-03 | WS7 | Discovery detail flyout: improved recommendations display | Recommendations had duplicated content (title, description, and steps all showing the same text). Now: title shown as normal-weight text (not bold), description only shown when it differs from title (rendered as markdown via `EuiMarkdownFormat`), steps only shown when they add unique content beyond the title. |
| 2026-03-03 | WS7 | Discovery detail flyout: compact evidence and bullet-list references | Evidence items collapsed from full `EuiDescriptionList` (5 rows each) into single compact lines: query title — stream · N events · change\_point · feature. References (queries, features, related discoveries) changed from comma-separated strings to `EuiListGroup` bullet lists. |
| 2026-03-03 | WS7 | Topology prompt rewritten for entities, infrastructure, dependencies, and color coding | Topology system prompt now instructs the LLM to focus on: (1) entities as primary nodes, (2) infrastructure as subgraph boundaries, (3) service dependencies with labeled edges, (4) color coding by importance — critical path (red), important services (blue), supporting (gray), external (purple) with explicit Mermaid style syntax. |
...
- `x-pack/platform/plugins/shared/streams/server/routes/internal/streams/topology/route.ts` — `GET _topology` (read persisted) + `POST _topology` (generate via LLM and persist to saved object). Now fetches active discoveries via `discoveryClient.searchDiscoveries()` and includes them in the LLM prompt for issue annotation on the diagram.
- `x-pack/platform/plugins/shared/streams/server/routes/index.ts` — registered topology routes
- `x-pack/platform/plugins/shared/streams/server/lib/saved_objects/significant_events/discovery_settings.ts` — added `topologyMermaid` field to `DiscoverySettingsAttributes` for topology persistence

### Streams App (UI)
- `x-pack/platform/plugins/shared/streams_app/public/components/significant_events_discovery/page.tsx` — added Overview (default), Suggestions, Topology tabs; updated imports for renamed files
- `x-pack/platform/plugins/shared/streams_app/public/components/significant_events_discovery/components/overview/overview_tab.tsx` — new: Overview landing page with Refresh button, 5 summary stats (discoveries, suggestions/pending, features, queries, streams), top 3 discoveries by relevance, top 3 recommendations by priority, top 3 suggestions by priority with ES|QL preview, persisted topology diagram at bottom
- `x-pack/platform/plugins/shared/streams_app/public/components/significant_events_discovery/components/discoveries/tab.tsx` — (renamed from `insights/tab.tsx`) simplified: "Generate discoveries" button + persisted discoveries table + detail flyout on row click. Detail flyout updated: description rendered as markdown (`EuiMarkdownFormat`), recommendations de-duplicated (title vs description vs steps), evidence compacted to single-line summaries, references rendered as `EuiListGroup` bullet lists.
- `x-pack/platform/plugins/shared/streams_app/public/components/significant_events_discovery/components/features_table/features_table.tsx` — added "Regenerate features" button that triggers `OnboardingStep.FeaturesIdentification` across all streams with existing features via `pMap` (concurrency 5); shows toast notifications for progress/errors; auto-refetches features after 10s
- `x-pack/platform/plugins/shared/streams_app/public/components/significant_events_discovery/components/queries_table/queries_table.tsx` — added "Regenerate queries" button and "Backfill (1h)" button; backfill calls `POST /internal/streams/queries/_backfill` and auto-refreshes after 15s
- `x-pack/platform/plugins/shared/streams_app/public/components/significant_events_discovery/components/queries_table/translations.ts` — added i18n translations for regenerate queries and backfill buttons, success/error/no-rules toast messages
- `x-pack/platform/plugins/shared/streams_app/public/hooks/use_queries_api.ts` — added `backfill()` method that calls `POST /internal/streams/queries/_backfill`

### Streams Plugin (Server) — Backfill
- `x-pack/platform/plugins/shared/streams/server/routes/internal/streams/backfill/route.ts` — new: `POST /internal/streams/queries/_backfill` route that fetches all promoted rule IDs via `queryClient.getQueryLinks([], { ruleBacked: true })` and calls `rulesClient.scheduleBackfill()` with configurable time range (default 1h lookback)
- `x-pack/platform/plugins/shared/streams/server/routes/types.ts` — added `rulesClient: RulesClient` to `RouteHandlerScopedClients`
- `x-pack/platform/plugins/shared/streams/server/routes/index.ts` — registered `internalBackfillRoutes`
