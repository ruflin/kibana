...
- `x-pack/platform/plugins/shared/streams_app/public/components/significant_events_discovery/components/queries_table/queries_table.tsx` — added "Regenerate queries" button that triggers `OnboardingStep.QueriesGeneration` across all streams with existing queries via `pMap` (concurrency 5); shows toast notifications for progress/errors; auto-invalidates query cache after 10s
- `x-pack/platform/plugins/shared/streams_app/public/components/significant_events_discovery/components/queries_table/translations.ts` — added i18n translations for regenerate queries button, success/error/no-streams toast messages
