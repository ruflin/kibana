...
run(({ log, flags }) => restoreEnvSnapshot({ log, flags }), {
  description: `
    ...
    Automates the full restore workflow:
      1.  Enable streams via Kibana API
      2.  Restore the significant events data streams (knowledge_indicators, discoveries, detections)
      ...
      4.  Ensure alert-index alias (.alerts-streams.alerts-default)
      5.  Repromote queries (resets rule_backed, then re-creates rules after restore)
