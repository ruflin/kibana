...
export async function callStreamsQueryUpsert(
  request: KibanaRequest,
  core: CoreStart,
  spaceId: string,
  streamName: string,
  ...
  const url = buildInternalUrl(
    ...
    core,
    spaceId,
    `/api/streams/${encodeURIComponent(streamName)}/queries/${encodeURIComponent(queryId)}`
  );
  ...
  if (!res.ok) {
    ...
    throw new Error(`Streams query upsert failed (${res.status}): ${text}`);
  }
  return res.json();
}
