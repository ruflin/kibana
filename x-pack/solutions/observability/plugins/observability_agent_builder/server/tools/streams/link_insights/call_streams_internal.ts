...
export async function callStreamsInsightLink(
  request: KibanaRequest,
  core: CoreStart,
  spaceId: string,
  streamName: string,
  insightUuid: string,
  body: { parent_insight_id?: string; related_insight_ids?: string[] }
): Promise<unknown> {
  const url = buildInternalUrl(
    request,
    core,
    spaceId,
    `/internal/streams/${encodeURIComponent(streamName)}/insights/${encodeURIComponent(insightUuid)}/_link`
  );
  ...
  if (!res.ok) {
    ...
    throw new Error(`Streams insight link failed (${res.status}): ${text}`);
  }
  return res.json();
}
