...
describe('write_query_knowledge_indicator tool', () => {
  ...
  it('uses the expected tool id', () => {
    ...
    const tool = createWriteQueryKnowledgeIndicatorTool({ getScopedClients, server, logger });

    expect(tool.id).toBe(STREAMS_WRITE_QUERY_KNOWLEDGE_INDICATOR_TOOL_ID);
    expect(tool.id).toBe('platform.streams.sig_events.write_query_knowledge_indicator');
  });
