...
describe('write_feature_knowledge_indicator tool', () => {
  ...
  it('uses the expected tool id', () => {
    ...
    const tool = createWriteFeatureKnowledgeIndicatorTool({ getScopedClients, server, logger });

    expect(tool.id).toBe(STREAMS_WRITE_FEATURE_KNOWLEDGE_INDICATOR_TOOL_ID);
    expect(tool.id).toBe('platform.streams.sig_events.write_feature_knowledge_indicator');
  });

  it('has confirmation policy askUser: once', () => {
    const getScopedClients = jest.fn() as unknown as jest.MockedFunction<GetScopedClients>;
    const tool = createWriteFeatureKnowledgeIndicatorTool({ getScopedClients, server, logger });

    expect(tool.confirmation).toEqual({ askUser: 'once' });
  });
