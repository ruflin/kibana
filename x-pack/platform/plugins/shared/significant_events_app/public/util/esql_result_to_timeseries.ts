...
export function esqlResultToTimeseries<T extends string>({
  ...
  result: AsyncState<ESQLSearchResponse>;
  metricNames: T[];
}): Array<Timeseries<T>> {
  ...
  rows.forEach((columnsInRow) => {
    ...
    values.forEach((value, key) => {
      if (key !== 'x') {
        (coordinate as Record<string, number | null>)[key] = value;
      }
    });
  ...
  return Array.from(collectedSeries.values()).map((timeseries) => ({
    ...timeseries,
    data: [...timeseries.data].sort((a, b) => a.x - b.x),
  }));
}
