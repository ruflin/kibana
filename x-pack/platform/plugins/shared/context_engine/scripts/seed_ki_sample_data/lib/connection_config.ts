...
export interface ConnectionConfig {
  esUrl: string;
  /** Empty when Kibana was not resolved (`--skip-kibana`). */
  kibanaUrl: string;
  username: string;
  password: string;
...
const assertBasicAuth = (auth: ConnectionAuth) => {
  if (isApiKeyAuth(auth)) {
    throw new Error('API key auth is not supported by seed_ki_sample_data.');
  }
  return auth;
};

/**
 * Resolves a superuser connection to Elasticsearch, and Kibana with the same user unless
 * `--skip-kibana` is set, for a local stateful or serverless stack or the cluster given by flags /
 * `ELASTICSEARCH_*` / `KIBANA_URL`.
 */
export async function getConnectionConfig(
  ...
  const esConfig = { esUrl: elasticsearch.url, esTls: elasticsearch.tls, username, password };

  if (flags['skip-kibana'] === true) {
    return { ...esConfig, kibanaUrl: '' };
  }
